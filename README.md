...
![Video Trimmer Demo](assets/trimmer_gif.gif)
...
dependencies:
flutter_video_trimmer: ^3.0.4


## 🛠️ Configuration

### Android Configuration
For Android, you need to modify the `AndroidManifest.xml` to request the required permissions for camera and storage access:
...
iOS Configuration
For iOS, add the following keys in your Info.plist to request the required permissions for camera, microphone, and photo library access:
...
Here’s a complete example showing how to build a custom video trimming screen using the `flutter_video_trimmer` package.
